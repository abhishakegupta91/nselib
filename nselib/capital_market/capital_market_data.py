...
from nselib.capital_market.get_func import (get_block_deals_data,
                                            get_bulk_deal_data,
                                            get_business_growth_cm_segment_daily,
                                            get_business_growth_cm_segment_monthly,
                                            get_business_growth_cm_segment_yearly,
...
from nselib.libutil import (
    ...
    validate_date_param, validate_param_from_list,
)
from nselib.request_maker import default_header, nse_urlfetch
from nselib.utils.enums import DateFormatEnum
from .constants import (BLOCK_DEALS_DATA_COLUMNS, BULK_DEAL_DATA_COLUMNS,
                        DELIVERABLE_DATA_COLUMNS, INDIA_VIX_DATA_COLUMN,
                        PRICE_VOLUME_AND_DELIVERABLE_POSITION_DATA_COLUMNS,
                        PRICE_VOLUME_DATA_COLUMNS, SHORT_SELL_DATA_COLUMNS, VAR_COLUMNS)
...
def price_volume_and_deliverable_position_data(
    ...
    while load_days > 0:
        if load_days > 365:
            end_date = (from_date + dt.timedelta(364)).strftime(DateFormatEnum.DD_MM_YYYY.value)
            start_date = from_date.strftime(DateFormatEnum.DD_MM_YYYY.value)
        else:
            end_date = to_date.strftime(DateFormatEnum.DD_MM_YYYY.value)
            start_date = from_date.strftime(DateFormatEnum.DD_MM_YYYY.value)
...
def india_vix_data(from_date: Optional[str] = None, to_date: Optional[str] = None,
                   period: Optional[str] = None):
    """
...
def bulk_deal_data(from_date: Optional[str] = None, to_date: Optional[str] = None,
                   period: Optional[str] = None):
    """
...
def block_deals_data(from_date: Optional[str] = None, to_date: Optional[str] = None,
                     period: Optional[str] = None):
    """
...
def short_selling_data(from_date: Optional[str] = None, to_date: Optional[str] = None,
                       period: Optional[str] = None):
    """
    ...
    nse_df = pd.DataFrame(columns=SHORT_SELL_DATA_COLUMNS)
    from_date = datetime.strptime(from_date, DateFormatEnum.DD_MM_YYYY.value)
    to_date = datetime.strptime(to_date, DateFormatEnum.DD_MM_YYYY.value)
    load_days = (to_date - from_date).days
    while load_days > 0:
...
def daily_volatility(trade_date: str):
    ...
    for url in report_urls:
        ...
        try:
            ...
            raise FileNotFoundError(
                f" CM daily volatility data not found for : {trade_date.strftime(DateFormatEnum.DD_MM_YYYY.value)} :: NSE error : {exc}"
            )
        data_df = data_df.dropna(how="all")
        data_df.columns = [column_name.strip() for column_name in data_df.columns]
        return data_df
    raise FileNotFoundError(
        f" No CM daily volatility data available for : {trade_date.strftime(DateFormatEnum.DD_MM_YYYY.value)} :: status={last_status_code}"
    )
...
def category_turnover_cash(trade_date: str):
    ...
    trade_date = datetime.strptime(trade_date, DateFormatEnum.DD_MM_YYYY.value)
    url = f"https://archives.nseindia.com/archives/equities/cat/cat_turnover_{trade_date.strftime(DateFormatEnum.DDMMYY.value)}.xls"
    file_chk = nse_urlfetch(url)
    if file_chk.status_code != 200:
        raise FileNotFoundError(f" No data available for : {trade_date}")
